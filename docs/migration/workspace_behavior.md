...
**Route:** `GET /case/:id/try/:try_number/new_ui` → `core#new_ui`, layout **`layouts/core_new_ui.html.erb`**, body **`app/views/core/new_ui.html.erb`** (query list via **`core/_query_list_shell.html.erb`**). Loads **`application_modern`** (importmap), not Angular bundles. The **top navbar** is the same **`layouts/_header`** partial as the rest of the app (Bootstrap 5): **`core_new_ui`** passes **`use_new_ui_shell: true`** so the cases Turbo frame uses **`/dropdown/cases?new_ui=true`** (links go to **`/case/…/new_ui`**) and **Create a case** posts to **`case_new_for_new_ui`**. Below the header, the **`core_new_ui`** CSS bundle (see **`app/assets/stylesheets/core_new_ui.css`**) still includes **`query_workspace_new_ui.css`** for workspace body chrome: Bootstrap 3–era layout and utilities the partials still expect (query container chrome, **`.result-header`**, **`.sub-results`**, **`#dev-settings`** east pane, **`pull-*`**, **`btn-default`**, footer padding helpers), plus **`search_results.css`** / **`stimulus_controllers.css`**. Core partials use **Bootstrap Icons** (`bi-*`) instead of Glyphicons. Expand/collapse chevrons use **`bi-chevron-down` / `bi-chevron-up`**; **`query-row`** toggles those classes on expand/collapse.
...
- **Expanded row toolbar:** Control order matches Angular **`searchResults.html`**: **Score All** (bulk-rate dropdown) first, then **Copy query** (icon-only), **Toggle Notes**, **Explain Query** + **Missing Documents** + **Match breakdown** (layers icon + label; per-document match breakdown / Solr–ES debug in the result grid — parity with Angular’s **Matches** / stacked-chart column, which needs explain payloads on each hit). Then **Set Options** (JSON modal, `PUT …/queries/:id/options`), **Move Query** + **Delete Query**. Bootstrap Icons replace Glyphicons. **Explain Query** runs a one-off debug request when needed for the **modal** (query-level parsing/params); **Match breakdown** re-runs the visible search with **`debug: true`** so result rows can render stacked explain bars like **`stackedChart`** in the legacy UI.
