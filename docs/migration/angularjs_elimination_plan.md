...
- ~~**`CaseCtrl` / `CurrSettingsCtrl` parity on `new_ui`**~~ — all responsibilities already covered: case rename (`inline_edit_controller.js`), case score tracking (`case_score_controller.js`), nightly toggle (`settings_panel_controller.js`), try rename/duplicate/delete (`settings_panel_controller.js`), try switching (History tab). **Header cases dropdown** now links to `new_ui` routes when rendered from `core_new_ui.html.erb` (via `use_new_ui` flag on `link_to_core_case`). **Create a case** from `new_ui` header uses separate `core#new_for_new_ui` action → redirects to `case_core_new_ui_path`. Angular default core unchanged.
...
- [x] **Frog report counts missing ratings across all docs, not scorer top-K:** ~~In Angular, `depthOfRating` comes from the scorer's `k` parameter (e.g., 10 for AP@10) and missing ratings are counted only in `docs.slice(0, depthOfRating)`.~~ Fixed: `scorer_executor.js` now captures `k` from scorer code via `recordDepthOfRanking()` (same pattern as Angular's `ScorerFactory`) and returns `{ score, depthOfRating }`. Both `query_row_controller._updateFrogIndicator()` and `frog_report_controller._collectQueryData()` use `depthOfRating` to slice docs before counting unrated.
...
- ~~**Stale books data in Judgements modal:**~~ Fixed: `judgements_controller.js` `open()` now calls `_refreshBooks()` which fetches fresh books from `api/teams/:id/books` for each team, matching Angular's `bookSvc.list(team)` pattern. Books are deduplicated and update the `booksValue` before rendering.
- ~~**Frog / debug / explain** — vega → vega-embed integration.~~ Done: `frog_report_controller.js` uses `vegaEmbed()` (from the vega gem's asset pipeline) for the Frog Pond Report bar chart. Explain stacked charts use inline SVG in `query_row_controller.js`, not Vega.
...
- [x] **TLS protocol switching:** ~~Angular detects when the search endpoint URL uses a different protocol (HTTP vs HTTPS) than Quepid itself, and offers to reload Quepid on the matching protocol (passing wizard state via URL params: `searchEngine`, `searchUrl`, `caseName`, `apiMethod`, `basicAuthCredential`).~~ Implemented in `wizard_controller.js`: `_checkTLS()` detects protocol mismatch on URL input change, shows a warning with a redirect link that carries wizard state via URL params. `_restoreStateFromUrlParams()` restores wizard state after the redirect. Warning is suppressed when Proxy Requests is enabled (the modern workaround). Strips dev port 3000 when switching to HTTPS.
- [x] **SearchAPI mapper code evaluation:** ~~When validating an existing SearchAPI endpoint in the wizard, Angular evaluates the mapper code with `new Function()` to check that `numberOfResultsMapper()` and `docsMapper()` functions exist before making a test search.~~ `validate()` in `wizard_controller.js` now requires mapper code for all SearchAPI endpoints (new and existing) and calls `validateMapperCode()` from `settings_validator.js` which uses `new Function()` to check that both `numberOfResultsMapper` and `docsMapper` functions are defined.
...
- [x] **Post-wizard tour auto-start:** ~~Angular triggers `setupAndStartTour()` 1500ms after the wizard closes if the user hasn't previously completed the case wizard.~~ `wizard_controller.js` `finish()` sets `?startTour=true` on reload URL only when `data-completed-case-wizard` is `false` (first-time users). `tour_controller.js` auto-starts on `?startTour=true` and cleans up the URL param via `history.replaceState()` to prevent restart on refresh. Layout exposes `completed_case_wizard` as a body `data-*` attribute.
...
- **2026-03-23** — **Phase 8 parity items complete** (3 of 4 remaining checkboxes): (1) TLS protocol switching — `wizard_controller.js` `_checkTLS()` detects HTTP/HTTPS mismatch between Quepid and search endpoint URL on input change, shows warning banner with redirect link carrying wizard state (`searchEngine`, `searchUrl`, `caseName`, `apiMethod`, `basicAuthCredential`). `_restoreStateFromUrlParams()` restores wizard state on the other side. Suppressed when proxy is enabled. (2) SearchAPI mapper code validation — `validate()` now requires and validates mapper code for all SearchAPI endpoints (new and existing) before probe search, using `validateMapperCode()` which checks `numberOfResultsMapper` and `docsMapper` via `new Function()`. (3) Post-wizard tour auto-start — `finish()` sets `?startTour=true` only for first-time users (`data-completed-case-wizard` body attribute from `core_new_ui.html.erb`). `tour_controller.js` cleans up `startTour` URL param via `history.replaceState()`. All 4 Phase 8 parity review items now resolved.
- **2026-03-23** — **Phase 2 CaseCtrl/CurrSettingsCtrl parity**: Audited Angular `CaseCtrl` (case selection, rename, scores, nightly) and `CurrSettingsCtrl` (try selection, rename) — all responsibilities already covered by Stimulus controllers on `new_ui`. Only gap was the **header cases dropdown** linking to Angular routes when on `new_ui`. Fix: `core_new_ui.html.erb` passes `use_new_ui: true` to `_header_core_app.html.erb`; `link_to_core_case` helper generates `case_core_new_ui_url` when flag is set, `case_core_url` otherwise. Added `core#new_for_new_ui` action + `GET /cases/new_ui` route for "Create a case" from `new_ui` header (redirects to `case_core_new_ui_path` with `showWizard=true`). Angular default core completely untouched.
