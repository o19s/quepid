...
class ExperimentWithBulkInsertTest < ActionDispatch::IntegrationTest
  ...
  test 'generate and export 5000 queries with traditional AR' do
    ...
    result = Benchmark.measure do
      50_000.times do
        ...
        qdp = book.query_doc_pairs.create query_text: query_text, doc_id: doc_id, document_fields: document_fields, information_need: information_need
        unless qdp.valid?
          puts "Failed to create qdp: #{qdp.errors.full_messages.join(', ')}"
        end
        assert qdp.valid?
      end
  ...
  test 'generate and export 5000 queries with bulk import' do
    ...
    result = Benchmark.measure do
      ...
      50_000.times do
        ...
        qdp = QueryDocPair.new book: book, query_text: query_text, doc_id: doc_id, document_fields: document_fields, information_need: information_need
        query_doc_pairs << qdp
  ...
  test 'generate and export 5000 queries with upsert_all when exists already data' do
    ...
    assert_equal 50000, book.query_doc_pairs.count   
