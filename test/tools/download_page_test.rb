...
class DownloadPageTest < ActiveSupport::TestCase
  ...
  test "accepts valid http urls" do
    ...
    refute result.include?("error"), "Should not contain error for valid response"
  end
  ...
  test "handles connection timeout" do
    ...
    refute result[:error].empty?, "Error message should not be empty"
  end
  ...
  test "strips css styling from html" do
    ...
    refute result.include?("<style>"), "Should remove style tags"
    refute result.include?("rel=\"stylesheet\""), "Should remove stylesheet links"
    refute result.include?("style="), "Should remove inline styles"
    refute result.include?("class="), "Should remove class attributes"
    refute result.include?("<script>"), "Should remove script tags"
    refute result.include?("onclick="), "Should remove event handlers"
    refute result.include?("javascript:"), "Should remove javascript URLs"
