...
class JavaScriptExtractorTest < ActionDispatch::IntegrationTest
  ...
  test "extracts javascript from labeled blocks" do
    ...
    refute result.include?('```'), "Should not contain backticks"
  ...
  test "ignores unlabeled non-javascript blocks" do
    ...
    refute result.include?('SELECT'), "Should not include SQL"
    refute result.include?('ruby_method'), "Should not include Ruby"
  ...
  test "works with fixture file" do
    ...
    assert result.include?('numberOfResultsMapper'), "Should contain numberOfResultsMapper function"
    assert result.include?('docsMapper'), "Should contain docsMapper function"
    refute result.include?('```'), "Should not contain backticks in extracted code"
  ...
  test "ignores empty code blocks" do
    ...
    refute result.include?("```"), "Should not contain backticks in result"
  end
