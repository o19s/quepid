...
# conflicting with the main quepid stack. We use nginx on 8080 (proxies to app) for reliability.
BASE_URL_DEFAULT="http://localhost:3000"
BASE_URL_VP="http://localhost:8080"
...
capture_branch() {
  ...
  # 4. Capture screenshots and API (run from main repo; app is at localhost:8080 via nginx for VP)
  cd "$GIT_ROOT"
