    ...
    module SearchEndpoints
      ...
      class ValidationsController < Api::ApiController
        ...
        def create
          ...
          if uri.scheme == "http"
            warnings << "Using HTTP instead of HTTPS — credentials will be sent in plain text"
          end
          ...
          begin
            ...
            if response.code.to_i >= 400 && response.code.to_i != 401 && response.code.to_i != 403
            ...
            if response.code.to_i == 401 || response.code.to_i == 403
              warnings << "Server requires authentication (HTTP #{response.code})"
            end
