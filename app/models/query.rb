...
class Query < ApplicationRecord
  ...
  # When we ask for a consolidated view of our ratings, then we create this "fake Rating"
  # object that has an averaged view.  Not sure why we don't just reuse the existing ActiveRecord
  # Rating object?
  # this method may not be needed if we just use the .average(:rating) on the ActiveRecord?
  def self.ratings_averaged ratings
    ratings_averaged = []
    unless ratings.empty?
      ratings_by_doc = CaseAnalyticsManager.group_by_doc_id_version_two(ratings)

      ratings_by_doc.each do |docid, ratings_for_docid|
        ratings_ratings = ratings_for_docid.map(&:rating).compact

        average_rating = if ratings_ratings.empty?
                           nil
                         else
                           mean(ratings_ratings).round(0)
                         end

        ratings_averaged << RatingAveraged.new(docid, ratings.first.query.id, average_rating)
      end
    end
    ratings_averaged
  end
