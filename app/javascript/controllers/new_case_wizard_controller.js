...
export default class extends Controller {
  ...
  onEngineChange() {
    ...
    this.searchapiHelpTarget.classList.toggle(
      'd-none',
      this.searchEngineTarget.value !== 'searchapi'
    );
    this._updateNavState();
  }
  ...
  async finish() {
    ...
    try {
      ...
      this._modal?.hide();
      const pathParts =
        this.tryNumberValue != null
          ? ['case', this.caseIdValue, 'try', this.tryNumberValue]
          : ['case', this.caseIdValue];
      const targetUrl = buildPageUrl(root, ...pathParts);
      window.location.href = `${targetUrl}?startTour=true`;
    } catch (err) {
  ...
  onCsvSelected() {
    ...
    reader.onload = (e) => {
      ...
      if (this.hasCsvPreviewTarget) {
        ...
        this.csvPreviewTarget.replaceChildren(
          this._buildCsvPreview('success', message, `Fields: ${result.fields.join(', ')}`)
        );
        this.csvPreviewTarget.classList.remove('d-none');
      }
      this._updateNavState();
  ...
  _clearShowWizardFromUrl() {
    if (getCurrentPageSearchParams().has('showWizard')) {
      window.history.replaceState({}, '', buildCurrentPageUrlWithParams({ showWizard: null }));
    }
  }
}
