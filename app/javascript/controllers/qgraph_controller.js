...
export default class extends Controller {
  ...
  scoresValueChanged()      { if (this._connected) this._render() }
  annotationsValueChanged() { if (this._connected) this._render() }
  maxValueChanged()         { if (this._connected) this._render() }
  ...
  _render() {
    ...
    const x = d3.scaleLinear().domain([0, scoreData.length - 1]).range([0, width])
    ...
    scoreData.forEach((d, i) => {
      ...
      const dateStr = d.updated_at ? new Date(d.updated_at).toLocaleDateString(undefined, { dateStyle: "medium" }) : ""
