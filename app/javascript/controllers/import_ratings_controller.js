...
export default class extends Controller {
  ...
  open(event) {
    ...
    this.formatRadioTargets.forEach((r) => {
      r.checked = false;
    });
  ...
  async submit(event) {
    ...
    if (isInfoNeeds) {
      ...
      if (!csvText) {
        this._showError(
          this.infoNeedsErrorTarget,
          'Please paste or upload CSV content (query,information_need).'
        );
        return;
      }
      this._hideError(this.infoNeedsErrorTarget);
      ...
      try {
        ...
        if (res.ok) {
          if (window.flash)
            window.flash.success = 'Successfully imported information needs from CSV.';
          this._modal?.hide();
        } else {
        ...
        this._showError(
          this.infoNeedsErrorTarget,
          err.message || 'Unable to import information needs.'
        );
      } finally {
    ...
    if (!format) {
      this._showError(
        this.ratingsErrorTarget,
        'Please select a format (CSV, RRE, or LTR) and provide content.'
      );
      return;
    }
    this._hideError(this.ratingsErrorTarget);
    ...
    if (format === 'csv') {
      ...
      if (!ratings.length) {
        this._showError(
          this.ratingsErrorTarget,
          'Could not parse CSV. Expected headers: query,docid,rating'
        );
        return;
      }
      ...
      const content =
        this._rreContent ||
        (this.hasRreFileTarget && this.rreFileTarget.files?.[0]
          ? await this._readFile(this.rreFileTarget.files[0])
          : null);
      if (!content) {
      ...
      const content =
        this._ltrContent ||
        (this.hasLtrFileTarget && this.ltrFileTarget.files?.[0]
          ? await this._readFile(this.ltrFileTarget.files[0])
          : null);
      if (!content) {
    ...
    try {
      ...
      if (res.ok) {
        if (window.flash)
          window.flash.success = `Successfully imported ratings from ${format.toUpperCase()}.`;
        this._modal?.hide();
      } else {
