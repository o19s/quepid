...
export default class extends Controller {
  ...
  static targets = [
    "modal", "trigger", "loadingEl", "noTeamsEl", "noBooksEl", "booksSection", "bookList",
    "createMissingQueries", "createBookLink", "createBookLinkFooter", "judgeLink", "refreshBtn", "primaryBtn",
    "errorEl"
  ]
  ...
  async open(event) {
    ...
    if (this._books.length === 0) {
      if (this.hasNoBooksElTarget) {
        ...
        const link = this.noBooksElTarget.querySelector("a[data-judgements-target='createBookLink']")
    ...
    createLinks.forEach((a) => { a.href = this.createBookUrlValue || "#" })
    this._syncButtons()
    this._modal.show()
  }
  ...
  _renderBookList() {
    ...
    noneLi.addEventListener("click", () => this._selectBook(null, null))
    ...
    for (const b of this._books) {
      ...
      li.addEventListener("click", () => this._selectBook(b.id, b.name))
      list.appendChild(li)
    }
  }
  ...
  _selectBook(id, name) {
    ...
    this.bookListTarget.querySelectorAll("li").forEach((li) => {
      li.classList.toggle("active", (li.dataset.bookId || null) === (id === null ? "" : String(id)))
    })
    this._syncButtons()
  }
  ...
  _syncButtons() {
    ...
    if (this.hasRefreshBtnTarget) {
      ...
      this.refreshBtnTarget.innerHTML = canRefresh && this._selectedBookName
  ...
  async _doRefresh(bookId, root) {
    const createMissing = this.hasCreateMissingQueriesTarget && this.createMissingQueriesTarget.checked
    ...
    try {
      ...
      this._modal?.hide()
      if (inBackground) {
        setTimeout(() => { window.location.href = buildPageUrl(root) }, 500)
      } else {
        window.location.reload()
      }
    } catch (err) {
