...
export default class extends Controller {
  ...
  async run() {
    ...
    const codeInput = this.hasCodeInputTarget ? this.codeInputTarget : this.element.querySelector("textarea[name*='[code]']")
    if (!codeInput) return
    ...
    try {
      ...
      const display = typeof score === "number" ? (Number.isInteger(score) ? score : score.toFixed(4)) : String(score)
