...
export default class extends Controller {
  ...
  _syncSubmitAndNotes() {
    ...
    const snapshotNoSelection = which === "snapshot" && (!this.hasSnapshotSelectTarget || !this.snapshotSelectTarget.value)
  ...
  async _fetchSnapshots() {
    ...
    try {
      ...
      this._snapshots = list.map((s) => ({ id: s.id ?? s.snapshot_id, name: s.name || s.display_name || `Snapshot ${s.id}` }))
  ...
  _fillSnapshotSelects() {
    const option = (s) => `<option value="${s.id}">${this._escapeHtml(s.name)}</option>`
  ...
  _setApiLinksHrefs() {
    ...
    if (this.hasApiCaseLinkTarget) this.apiCaseLinkTarget.href = buildApiUrl(root, "cases", `${cid}.json`) + "?shallow=false"
    if (this.hasApiQueriesLinkTarget) this.apiQueriesLinkTarget.href = buildApiUrl(root, "cases", cid, "queries.json") + "?shallow=false"
    if (this.hasApiAnnotationsLinkTarget) this.apiAnnotationsLinkTarget.href = buildApiUrl(root, "cases", cid, "annotations.json")
    if (this.hasApiScoresLinkTarget) this.apiScoresLinkTarget.href = buildApiUrl(root, "cases", cid, "scores.json")
    if (this.hasApiRatingsLinkTarget) this.apiRatingsLinkTarget.href = buildApiUrl(root, "export", "ratings", `${cid}.json`)
    if (this.hasQuepidFormatLinkTarget) this.quepidFormatLinkTarget.href = buildApiUrl(root, "export", "cases", `${cid}.json`)
    this._updateApiSnapshotLink()
  }

  _updateApiSnapshotLink() {
    if (!this.hasApiSnapshotSelectTarget || !this.hasApiSnapshotLinkTarget || !this.hasApiSnapshotLinkWrapTarget) return
    ...
    const root = this.rootUrl
    this.apiSnapshotLinkTarget.href = buildApiUrl(root, "cases", this.caseIdValue, "snapshots", `${sid}.json`) + "?shallow=false"
  ...
  async submit() {
    ...
    if (ASYNC_FORMATS.includes(which)) {
      const sid = which === "snapshot" && this.hasSnapshotSelectTarget ? this.snapshotSelectTarget.value : ""
    ...
    switch (which) {
      case "information_need":
        ...
        filename = `${caseName}_information_need.csv`
        break
      case "basic": {
        ...
        filename = sid ? `${caseName}_basic_snapshot.csv` : `${caseName}_basic.csv`
        break
      }
      case "trec": {
        const sid = this.hasBasicSnapshotSelectTarget ? this.basicSnapshotSelectTarget.value : ""
        ...
        filename = sid ? `${caseName}_trec_snapshot.txt` : `${caseName}_trec.txt`
        break
      }
      case "rre":
