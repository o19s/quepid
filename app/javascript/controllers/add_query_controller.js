...
export default class extends Controller {
  ...
  connect() {
    this._boundUpdateButtonState = () => this._updateButtonState();
    this._updateButtonState();
    this.inputTarget.addEventListener('input', this._boundUpdateButtonState);
  }

  disconnect() {
    if (this.hasInputTarget) {
      this.inputTarget.removeEventListener('input', this._boundUpdateButtonState);
    }
  }
  ...
  _updateButtonState() {
    ...
    this.submitButtonTarget.textContent = hasSemicolon ? 'Add queries' : 'Add query';
  }
  ...
  _postOne(root, caseId, queryText, token) {
    ...
    return fetch(url, {
      ...
      if (!res.ok) {
        ...
        return res
          .json()
          .then((body) => Promise.reject(new Error(body.error || body.message || res.statusText)));
      }
  ...
  _postBulk(root, caseId, queries, token) {
    ...
    return fetch(url, {
      ...
      if (!res.ok)
        return res.json().then((body) => Promise.reject(new Error(body.error || res.statusText)));
      return res;
    });
  }
