...
export default class extends Controller {
  ...
  static targets = ["modal", "trigger", "selectionContainer", "addBtn", "applyBtn", "clearBtn", "loadingEl", "duplicateWarning"]
  ...
  _renderSelections() {
    ...
    this._selections.forEach((val, index) => {
      ...
      select.addEventListener("change", (e) => this._selectionChanged(index, e.target.value))
      ...
      clearBtn.addEventListener("click", () => this._removeSelection(index))
  ...
  _dispatch(snapshotIds) {
    ...
    workspace.dispatchEvent(new CustomEvent("diff-snapshots-changed", {
  ...
  apply(event) {
    ...
    if (this.hasDuplicateWarningTarget && !this.duplicateWarningTarget.classList.contains("d-none")) {
      return
    }
    this._dispatch(ids)
    if (window.flash) {
      window.flash.success = ids.length === 0
