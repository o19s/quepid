...
class JavaScriptExtractor < RubyLLM::Tool
  ...
  def execute markdown_content:
    # Validate input
    unless markdown_content.is_a?(String)
      return { error: "Invalid input: markdown_content must be a string" }
    end

    if markdown_content.strip.empty?
      return { error: "Empty markdown content provided" }
    end
  ...
  def extract_javascript_blocks(content)
    ...
    if javascript_blocks.empty?
      content.scan(/```\s*\n(.*?)\n```/m) do |match|
        ...
        if js_patterns.any? { |pattern| code.match?(pattern) }
  ...
  def validate_javascript_syntax(code)
    ...
    unless brace_balance == 0
      return { valid: false, error: "Unbalanced braces (#{brace_balance > 0 ? 'missing' : 'extra'} closing brace)" }
    end
    
    unless bracket_balance == 0
      return { valid: false, error: "Unbalanced brackets (#{bracket_balance > 0 ? 'missing' : 'extra'} closing bracket)" }
    end
    
    unless paren_balance == 0
      return { valid: false, error: "Unbalanced parentheses (#{paren_balance > 0 ? 'missing' : 'extra'} closing parenthesis)" }
    end
