...
# rubocop:disable Metrics/BlockLength
namespace :erd do
  desc 'Generate Entity Relationship Diagram image at docs/erd.png and embed it in docs/data_mapping.md'
  task image: :environment do
    doc_path = 'docs/data_mapping.md'
    ...
    block = "<!-- BEGIN ERD: generated by `rake erd:image`, do not edit by hand -->\n" \
            "![Quepid data model](erd.png)\n" \
            '<!-- END ERD -->'

    doc = File.read(doc_path)
    doc.sub!(/<!-- BEGIN ERD.*?<!-- END ERD -->/m, block) ||
      abort("Could not find the ERD markers in #{doc_path}")
    File.write(doc_path, doc)

    puts 'Updated ERD diagram at docs/erd.png and embedded it in docs/data_mapping.md'
  end
end
# rubocop:enable Metrics/BlockLength
