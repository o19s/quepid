...
function buildController() {
  ...
  ;['default', 'name', 'name_desc', 'score_asc', 'score_desc', 'modified', 'modified_desc', 'error']
    .forEach((value) => {
...
describe('query_list_controller', () => {
  ...
  describe('sort', () => {
    it('sorts by name ascending', () => {
      ...
      controller._originalOrder = Array.from(controller.listTarget.querySelectorAll('[data-query-id]'))
    ...
    it('sorts by score ascending', () => {
      ...
      const items = Array.from(controller.listTarget.querySelectorAll('[data-query-id]'))
      expect(parseFloat(items[0].dataset.queryScore)).toBeLessThanOrEqual(parseFloat(items[1].dataset.queryScore))
    })
    ...
    it('sorts by score descending', () => {
      ...
      const items = Array.from(controller.listTarget.querySelectorAll('[data-query-id]'))
      expect(parseFloat(items[0].dataset.queryScore)).toBeGreaterThanOrEqual(parseFloat(items[1].dataset.queryScore))
    })
    ...
    it('restores original order when sorting by default', () => {
      ...
      const items = Array.from(controller.listTarget.querySelectorAll('[data-query-id]'))
      expect(items.map(el => el.dataset.queryId)).toEqual(originalOrder.map(el => el.dataset.queryId))
    })
  })
