...
describe('Service: queriesSvc', function () {
  ...
  describe('- Scorer Tests: ', function() {
    it('uses default scorer when none present', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      expect(qResp.queries[0].hasOwnProperty('scorerId')).toBeFalsy();
      setupQuerySvc(undefined, qResp);
      var testQuery = queriesSvc.queries['0'];
      expect(testQuery.effectiveScorer()).toBe(mockScorerSvc.defaultScorer);
      expect(testQuery.scorerEnbl).toBe(false);
    });

    it('uses default scorer when scorerId Null', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      expect(qResp.queries[0].hasOwnProperty('scorerId')).toBeFalsy();
      qResp.queries[0].scorerId = null;
      qResp.queries[0].scorerEnbl = false;
      setupQuerySvc(undefined, qResp);
      var testQuery = queriesSvc.queries['0'];
      expect(testQuery.effectiveScorer()).toBe(mockScorerSvc.defaultScorer);
      expect(testQuery.scorerEnbl).toBe(false);
      expect(testQuery.scorerId).toBe(null);
    });

    it('uses default scorer when cant find scorer', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      qResp.queries[0].scorerId = 1;
      qResp.queries[0].scorerEnbl = true;
      setupQuerySvc(undefined, qResp);
      var testQuery = queriesSvc.queries['0'];
      expect(testQuery.effectiveScorer()).toBe(mockScorerSvc.defaultScorer);
      expect(testQuery.scorerEnbl).toBe(false);
    ...
    it('uses default scorer when custom scoring disbl', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      var scorer = mockScorerSvc.createScorer('bananas');
      mockScorerSvc.create(scorer);

      qResp.queries[0].scorerId = scorer.scorerId;
      qResp.queries[0].scorerEnbl = false;
      setupQuerySvc(undefined, qResp);
      var testQuery = queriesSvc.queries['0'];
      expect(testQuery.effectiveScorer()).toBe(mockScorerSvc.defaultScorer);
      expect(testQuery.scorerEnbl).toBe(false);
      expect('' + testQuery.scorerId).toBe('' + scorer.scorerId);
    });

    it('uses assigned scorer when found', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      var scorer = mockScorerSvc.createScorer('bananas');
      mockScorerSvc.create(scorer);
      ...
      qResp.queries[0].scorerId   = scorer.scorerId;
      qResp.queries[0].scorerEnbl = true;
      qResp.queries[0].scorer     = scorer;

      setupQuerySvc(undefined, qResp);
      var testQuery = queriesSvc.queries['0'];

      expect('' + testQuery.scorerId).toBe('' + scorer.scorerId);
      expect(testQuery.effectiveScorer().id).toBe(scorer.id);
      expect(testQuery.scorerEnbl).toBe(true);
    });

    it('assigns to new scorer from preexisting', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      var scorer = mockScorerSvc.createScorer('bananas');
      mockScorerSvc.create(scorer);

      var otherScorer = mockScorerSvc.createScorer('apples');
      mockScorerSvc.create(otherScorer);

      var yetAnotherScorer = mockScorerSvc.createScorer('ooples AND banoonoos');
      mockScorerSvc.create(yetAnotherScorer);
      ...
      // assign once
      var testQuery = queriesSvc.queries['0'];
      $httpBackend.expectPUT('/api/cases/2/queries/' + testQuery.queryId + '/scorer')
      .respond(201);
      var qVersionBefore = testQuery.version();
      var called = 0;
      var qSvcVersionBefore = queriesSvc.version();
      testQuery.saveScorer(otherScorer)
      .then(function() {
        called++;
        expect(testQuery.scorer).toBe(otherScorer);
        expect(testQuery.scorerEnbl).toBe(true);
        expect(testQuery.version()).not.toEqual(qVersionBefore);
        expect(queriesSvc.version()).not.toEqual(qSvcVersionBefore);
      });
      $httpBackend.flush();
      expect(called).toEqual(1);
    });

    it('unassigns scorers', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      var scorer = mockScorerSvc.createScorer('bananas');
      mockScorerSvc.create(scorer);
      ...
      // check for sane assignment, but don't wait out
      // the promise
      expect('' + testQuery.scorerId).toBe('' + scorer.scorerId);
      // explicity work around the promise
      testQuery.scorer = scorer;
      $httpBackend.expectDELETE('/api/cases/2/queries/' + testQuery.queryId + '/scorer')
      .respond(200);
      testQuery.unassignScorer()
      .then(function() {
        called++;
        expect(testQuery.effectiveScorer()).toBe(mockScorerSvc.defaultScorer);
        expect('' + testQuery.scorerId).toBe('' + scorer.scorerId);
    ...
    it('saves a test scorer for a query', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      setupQuerySvc(undefined, qResp);
      var testQuery = queriesSvc.queries['0'];
      var scorer    = {
        ...
        'queryTest':  false,
        'queryId':    testQuery.queryId,
      };

      testQuery.saveTest(scorer);
      $httpBackend.verifyNoOutstandingExpectation();

      expect(testQuery.test).not.toBe(null);
    });

    it('updates test scorer for a query', function() {
      var qResp = angular.copy(mockFullQueriesResp);
      setupQuerySvc(undefined, qResp);
      var testQuery = queriesSvc.queries['0'];
      var scorer    = {
      ...
      expect(testQuery.test).not.toBe(null);
      expect(testQuery.test.code).toEqual('fail();');
    });
  });
