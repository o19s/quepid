class ConvertOptionsColumnToJson < ActiveRecord::Migration[8.0]
  def up
      # First, create a temporary column
      add_column :queries, :options_json, :json
  
      # Copy and convert data from the string column to JSON
      Query.where.not(options: nil).find_each do |record|
      ...
      rename_column :queries, :options_json, :options
    end
    ...
    def down
      ...
      remove_column :queries, :options
      rename_column :queries, :options_string, :options
    end
end
